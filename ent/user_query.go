...
import (
	...
	"fmt"
	"math"
	"tutorial_graphql_go/ent/predicate"
	"tutorial_graphql_go/ent/todo"
	"tutorial_graphql_go/ent/user"
...
func (uq *UserQuery) loadTodos(ctx context.Context, query *TodoQuery, nodes []*User, init func(*User), assign func(*User, *Todo)) error {
	...
	for _, n := range neighbors {
		fk := n.todo_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "todo_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "todo_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
